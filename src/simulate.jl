...
mutable struct Direct <: AbstractSignal
    ...
    sc_r::typeof(0.0km) # radial distance from center of moon of the spacecraft for this event in Moon centric coordinates
...
mutable struct Reflected <: AbstractSignal
    ...
    triggered::Bool # did this event trigger
    sc_r::typeof(0.0km) # radial distance from center of moon of the spacecraft for this event in Moon centric coordinates (rad)
    sc_θ::Float64 # Lunar-centric theta angle of spacecraft (rad)
    sc_ϕ::Float64 # Lunar-centric phi angle of spacecraft (rad)
end
...
function compute_direct(::ScalarGeometry,
    ...
    sc_θ, sc_ϕ, sc_r = cartesian_to_spherical(antenna)
    ...
    return Direct(Ecr, rad2deg(θ), rad2deg(ϕ), rad2deg(zenith),
        ...
        depth, Drego, Dvacuum, rad2deg(θ_i), rad2deg(θ_emit), tpar, tperp, false, sc_r, sc_θ, sc_ϕ)
...
function compute_reflected(::ScalarGeometry,
    ...
    return Reflected(Ecr, rad2deg(θ), rad2deg(ϕ), rad2deg(zenith),
        ...
        tpar, tperp, rpar, rperp, subpar, subperp, false, sc_r, sc_θ, sc_ϕ)
