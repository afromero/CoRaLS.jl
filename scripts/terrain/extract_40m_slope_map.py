#!/usr/bin/env python3
"""Extract the native 40 m/pixel LDSM slope-angle map.

Although its filename ends in ``5mpp``, the source LDSM BigTIFF stores slope
angles in degrees, not elevations. Overview level 3 is its native 40 m/pixel
slope map. This script writes a compressed NPZ for the Julia location-aware
slope model and can optionally also write an independent GeoTIFF.
"""
...
DEFAULT_OUTPUT = REPOSITORY_ROOT / "data" / "ldsm_87s_slope_40mpp.tif"
DEFAULT_NPZ_OUTPUT = REPOSITORY_ROOT / "data" / "ldsm_87s_slope_40mpp.npz"
OVERVIEW_LEVEL = 3
...
def main() -> None:
    ...
    if slope_deg.shape != expected_shape or pixel_size_m != 40.0:
        raise RuntimeError(
            f"expected the 40 m overview to be {expected_shape}; got "
